...
PATH2AUX='~/bin/FILM2';
addpath([PATH2AUX '/utils/PhaseRnd']) 
...
[Yi,InputImgStat]=CleanNIFTI_spm(randtempfilename,'demean');
...
if size(Yi,1)~=Ti; Yi = Yi'; end; %TxV

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% SIMULATE DATA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp('== Simulate the data using phase randomisation ==')

SourceY = Yi(1:T,:);
Y       = ThetaRnd(SourceY,T,1,SIDX);
[sT,sV] = size(Y);
disp(['fMRI of time ' num2str(sT) ' & voxel ' num2str(sV) ' has been simulated.'])

%[Xcheck,Ycheck] = DrawMeSpectrum(Y,TR);
%%% DETREND %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
MCp = [];
if MParamNum
    MCp = load(Path2MC); % if MParamNum     == 6
    MCp = MCp(1:T,:); % cut of the motions for the simulated data
    if MParamNum == 12
        MCp = [MCp,MCp.^2]; % 12 parameter motion 
    elseif MParamNum == 24
        o6MCp   = MCp;%load(Path2MC);  % 6 orig param
