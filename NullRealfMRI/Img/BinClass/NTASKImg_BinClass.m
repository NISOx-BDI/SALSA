...
FigDir = '/Users/sorooshafyouni/Home/GitClone/FILM2/NullRealfMRI/Img/BinClass/RFigs10';
...
pwmethodlist = {'ACF','gFAST','gACFadjxACFadj2tT1S0P5','AR-W'}; %{'ACF','ACFadj','gFAST','ACFadjx2','AR-W'};
    ...
    for dtmethod = dtmethodlist
        for AROcnt = 1:numel(AROlist)
            ...
            for s = 1:SubNum %s = SubList'
                ...
                A=[Path2Mats 'R.PW/T' num2str(T) '/' CohortID '_NTASK_' num2str(TR*1000) '_' num2str(T) '_' pwmethod{1} '_AR-' num2str(AROlist(AROcnt)) '_MA-0_FWHM' num2str(FWHMl) '_' dtmethod{1} '_gsr' num2str(gsrflag) '_aroma' num2str(icaflag) '/' SubList{s} '_' SesList{s}];
                %EDERF_0_ACF_AR20_MA0_FWHM5_poly4_wtv_ICACLEAN1_GSR0.nii
                A=[A '/NTASK_SNR_' taskSNR '_ED'  EDType '_' num2str(BCl) '_' pwmethod{1} '_AR' num2str(AROlist(AROcnt)) '_MA0_FWHM' num2str(FWHMl) '_' dtmethod{1} num2str(dtla(dt_cnt)) '_ICACLEAN' num2str(icaflag) '_GSR' num2str(gsrflag) '.mat'];
    ...
    for pwmethod = pwmethodlist %{'AR-W','ACF','ACFadj'}%,'ARMAHR'}
        ...
        for dtmethod = dtmethodlist
            for AROcnt = 1:numel(AROlist)
                ...
                swBIN                         = BIN{dt_cnt}{pwm_cnt}{AROcnt}; 
