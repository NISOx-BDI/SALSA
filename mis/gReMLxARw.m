...
% tcon(2)  = 1;
% 
% [cbhat,RES,stat,se,tv,zv,Wcbhat,WYhat,WRES,wse,wtv,wzv] = gReMLxARw(Y,X,TR,tcon,-2,ImgStat,path2mask,1,K);
% 
...
function [cbhat,RES,stat,se,tv,zv,Wcbhat,WYhat,WRES,WBLUSRES,wse,wtv,wzv] = gReMLxARw(Y,X,TR,tcon,ARO,ImgStat,path2mask,K,WMSeg,J)
...
% 
    ...
    disp(['gReMLxARw:: fit the intial naive model.'])
    ...
    if ~exist('WMSeg','var')
        disp(['gReMLxARw:: fit gloabl FAST.'])
        [WY,WX]           = gReML(Y,X,TR,tcon,0); 
        clear X Y
        disp(['gReMLxARw:: fit voxel-wise prewhitening.'])
        [~,~,~,~,~,~,Wcbhat,WYhat,WRES,WBLUSRES,wse,wtv,wzv] = arw(WY,WX,tcon,ARO,ImgStat,path2mask,K);
        %[~,~,~,~,~,~,Wcbhat,WYhat,WRES,WBLUSRES,wse,wtv,wzv] = arw(Y,X,tcon,ARO,ImgStat,path2mask,K)
    else exist('WMSeg','var')
        disp('gReMLxARw:: prewhitening is being done on segemnts differently.')
        disp('gReMLxARw:: No ACF smoothing will be done.')
        ...
        disp(['gReMLxARw:: total # of voxels: ' num2str(nvox)])
        disp(['gReMLxARw:: # of GM voxels: ' num2str(sum(Idx_gm)) ', # of WM voxels: ' num2str(sum(Idx_wm))])
        
        disp('gReMLxARw:: apply gFAST on grey matter')
        [WYgm,WXgm]       = gReML(Ygm,X,TR,tcon,[],J);
        
        disp('gReMLxARw:: apply feat5 on the grey matter & CSF')
        [~,~,~,~,~,~,Wcbhat_gm,WYhat_gm,WRES_gm,WBLUSRES_gm,wse_gm,wtv_gm,wzv_gm] = arw(WYgm,WXgm,tcon,ARO,ImgStat,[],[]);
        
        disp('gReMLxARw:: apply feat5 on the white matter.')
        [~,~,~,~,~,~,Wcbhat_wm,WYhat_wm,WRES_wm,WBLUSRES_wm,wse_wm,wtv_wm,wzv_wm] = arw(Ywm,X,tcon,ARO,ImgStat,[],K);
