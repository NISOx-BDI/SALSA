...
if [ $betflag == 1 ]; then
	...
	echo ""
	echo "Find a robust FOV"
	${FSLDIR}/bin/robustfov -i ${fMRIPREP}/${ANATIMGNAME} -r ${fMRIPREP}/${ANATIMGNAME}

	echo "Bet the image."
	${FSLDIR}/bin/bet ${fMRIPREP}/${ANATIMGNAME} ${fMRIPREP}/${ANATIMGNAME}_brain -f 0.3 -n -m -R -S -B
fi

if [ $flag_feat1 == 1 ]; then
	echo "Runing the first bit of feat..."

	${FSLDIR}/bin/fslmaths ${FUNCIMG} prefiltered_func_data -odt float #copy fun image to the current directory
